...
namespace ModelWPF.Game.PathFinder
{
    public class PushPathFinder
    {
		readonly Location destination;
		readonly CellWPF startCell;
		...
		public PushPathFinder(CellWPF start, Location destination)
		{
		...
		public bool TryFindPath()
		{
			...
			if (horizontal)
			{
				...
				if (current.ColumnNumber < destination.ColumnNumber)
				{
					...
					{
						CellWPF cell = level[rowNumber, i];
			...
			else /* Vertical path. */
			{
				...
				if (current.RowNumber < destination.RowNumber)
				{
					...
					{
						CellWPF cell = level[i, columnNumber];
		...
		/// <returns></returns>
		bool Enter(CellWPF cell, Direction fromDirection)
		{
			bool result = false;
			if (cell.CanEnter)
			{
			...
			else if (!treasurePushed && cell.CellContents is TreasureWPF)
			{
				treasurePushed = true;
				result = true;
			}
